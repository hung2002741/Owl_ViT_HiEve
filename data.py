...
class OwlDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        w, h = image.size
        metadata = {"width": w, "height": h, "impath": path}
        ...
        # Compute remaining prompts
        remaining = list(set(self.label_text2id.keys()) - set(text_promts))
        k = self.max_promt_length - len(text_promts)
        if remaining and k > 0:
            remaining = random.choices(remaining, k=k)
        else:
            remaining = []  # No remaining prompts to add
        new_text_promts = text_promts + remaining

        # Apply random prompts
        ...
        transform = transforms.Compose([
            ...
            transforms.RandomHorizontalFlip(p=0.2) if self.is_train else transforms.Lambda(lambda x: x),
            transforms.RandomRotation(degrees=30) if self.is_train else transforms.Lambda(lambda x: x),
            transforms.ToTensor(),
        ])
